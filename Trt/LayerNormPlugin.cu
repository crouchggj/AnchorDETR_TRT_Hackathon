...
__global__ void layerNormKernel(float* pInput, float* pMul, float* pAdd,
                                float* pOutput) {
  ...
  temp[tx] =
      (value0 - mean) * (value0 - mean) + (value1 - mean) * (value1 - mean);
  __syncthreads();
  ...
  pOutput[index] = ((value0 - mean) * rsqrtf(var + 6e-6)) * pMul[tx] + pAdd[tx];
  pOutput[index + 128] =
      ((value1 - mean) * rsqrtf(var + 6e-6)) * pMul[tx + 128] + pAdd[tx + 128];
}
...
int32_t LayerNormPlugin::enqueue(const PluginTensorDesc* inputDesc,
  ...
  if (inputDesc[0].type == DataType::kHALF) {
    invokeGeneralLayerNorm((half*)outputs[0], (half*)inputs[0],
                           (half*)inputs[1], (half*)inputs[2], nBlock, 256,
                           stream, 0);
  } else if (inputDesc[0].type == DataType::kFLOAT) {
    invokeGeneralLayerNorm((float*)outputs[0], (float*)inputs[0],
                           (float*)inputs[1], (float*)inputs[2], nBlock, 256,
                           stream, 0);
  } else {
