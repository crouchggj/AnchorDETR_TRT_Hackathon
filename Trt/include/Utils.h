...
class Calibrator : public nvinfer1::IInt8Calibrator {
 public:
  Calibrator(int nBatchSize, BuildEngineParam *pParam,
      ...
      : nBatchSize(nBatchSize),
        nValue(nBatchSize * pParam->nChannel * pParam->nHeight *
               pParam->nWidth),
  ...
  virtual bool getBatch(void *adpInput[], const char *aszInputName[],
    ...
    std::stringstream frame_id_str;
    frame_id_str << "batch_";
    frame_id_str << iRound++ << ".npy";
    xt::xarray<float> output_map =
        xt::load_npy<float>(kCalibrationDataFolder + "/" + frame_id_str.str());
    ck(cudaMemcpy(dpInput, output_map.data(), output_map.size() * sizeof(float),
 ...
 private:
  ...
  const std::string kCalibrationDataFolder = "data/calibration";
};
